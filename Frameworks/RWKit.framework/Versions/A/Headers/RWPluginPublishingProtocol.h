...
#pragma mark - Attributed String Options

/// Options to be used when generating content from attributed strings
typedef NS_OPTIONS(NSInteger, AttributedStringExportOption) {
    // Attachments will have a relative path
    AttributedStringExportOptionStripPath = 1 << 0,
    
    // Attachments will have a fully qualified path
...
@protocol RWPluginPublishingProtocol

/// Called to obtain published content. Use methods on RWPluginPublishingContext to supply page content
/// May be called on any thread, do not block the main thread in this method
...
/// An object containing all parameters used during preview or export
